...
public class MyToneAnalyzer : MonoBehaviour {
    ...
    private int runID = -1;
    private bool analyzeMade = true;

    void Start()
    {
        analyzeString = "Hey, this is a test!";
        analyzeFlag = false;
    ...
    private void Update()
    {
        if(analyzeFlag && analyzeMade)
        {
            analyzeMade = false;
            Runnable.Run(AnalyzeGiven());
            analyzeFlag = false;
        }
    }
    ...
    private IEnumerator AnalyzeGiven()
    {
        if (!_service.GetToneAnalyze(OnGetToneAnalyze, OnFail, analyzeString))
